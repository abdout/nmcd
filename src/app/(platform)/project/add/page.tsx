...
import { useProject } from "@/components/platform/project/context";
import { useState } from "react";
import { useUpload } from "@/components/upload/context";
import SelectPopover from "@/components/atom/popover/popover";
import { lead, leads, status, statuses } from "@/components/platform/project/constant";
...
const CreateProject: React.FC<CreateProps> = ({ onClose }) => {
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (response.ok) {
        // Reset form and state AFTER a successful response
        form.reset();
        ...
        // Use router.push to navigate only after all steps are done
        router.push('/project');
      } else {
    ...
    <div className="flex items-top h-screen w-5/6">
      <Form {...form}>
        <form
          ...
          className={`${step === 1 ? 'w-screen px-40 pr-60 pt-20' : 'w-screen px-32 pt-10 items-top justify-center '}`}
        >
          {step === 1 && (
